...
async function GetReports(reportType, id) {
    const report = await contract.evaluateTransaction('GetReports', reportType , id);
...
router.post('/list', (request, response) => {
    let reportType = request.body.type;
    let id = request.body.id;

    GetReports(reportType,id)
       .then((result) => {
        response.send(JSON.parse(result.toString()));
    })
    .catch((error) => {
...
/*router.post('/reports/create', (request, response) => {
