...
function printHelp() {
  ...
  println "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Org1 ,Org2 , Org3, Org4, Org5"
...
function createOrgs() {
  ...
  infoln "Generate CCP files for Org1 ,Org2 , Org3, Org4, Org5"
...
## call the script to join create the channel and join the peers of Org1 ,Org2 , Org3, Org4, Org5
...
function networkDown() {
  # stop org3 containers also in addition to Org1 ,Org2 , Org3, Org4, Org5, in case we were running sample to add org3
